...
usage() {
  cat <<'EOF'
Usage:
  bash native-vast.sh --pool HOST:PORT --wallet PAYOUT --worker NAME [--tls] [--gpu INDEX]
...
start_native_instance() {
  ...
  if [[ "$POOL_TLS" == true ]]; then
    controller_tls_args+=(--tls)
    [[ "$POOL_TLS_INSECURE" == true ]] && controller_tls_args+=(--tls-insecure)
  fi
  runtime_ld_library_path="$(native_runtime_ld_library_path)"
